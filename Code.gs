...
const defaultData = {
  categories: [
    { id: crypto.randomUUID(), name: "Income", type: "Add", budget: 0 },
    { id: crypto.randomUUID(), name: "Food", type: "Deduct", budget: 800 },
    { id: crypto.randomUUID(), name: "Rent", type: "Deduct", budget: 1800 },
    { id: crypto.randomUUID(), name: "Utilities", type: "Deduct", budget: 450 },
    { id: crypto.randomUUID(), name: "Transport", type: "Deduct", budget: 350 },
    { id: crypto.randomUUID(), name: "Savings", type: "Deduct", budget: 600 }
  ],
  transactions: []
};
...
function seedTransactions(categories) {
  ...
  return [
    { id: crypto.randomUUID(), date: iso(0), description: "Salary", categoryId: find("Income"), amount: 5800 },
    { id: crypto.randomUUID(), date: iso(0), description: "Rent", categoryId: find("Rent"), amount: 1800 },
    { id: crypto.randomUUID(), date: iso(1), description: "Grocery Store", categoryId: find("Food"), amount: 220 },
    { id: crypto.randomUUID(), date: iso(2), description: "Electricity Bill", categoryId: find("Utilities"), amount: 160 },
    { id: crypto.randomUUID(), date: iso(3), description: "Metro Card", categoryId: find("Transport"), amount: 95 },
    { id: crypto.randomUUID(), date: iso(4), description: "Transfer to Savings", categoryId: find("Savings"), amount: 500 },
    { id: crypto.randomUUID(), date: iso(5), description: "Coffee Shop", categoryId: find("Food"), amount: 28 },
    { id: crypto.randomUUID(), date: iso(6), description: "Freelance Payment", categoryId: find("Income"), amount: 950 },
    { id: crypto.randomUUID(), date: iso(8), description: "Internet", categoryId: find("Utilities"), amount: 210 },
    { id: crypto.randomUUID(), date: iso(10), description: "Taxi", categoryId: find("Transport"), amount: 42 }
  ];
}
...
function isSafeDate(date) {
  ...
  // Keep dates inside a normal budgeting range so bad Sheet values like 4648 or 45810 do not break Intl.DateTimeFormat.
...
function parseAppDate(dateValue) {
  ...
  // Accept common Sheet display format: 01/06/2026 or 1/6/2026.
  // This app treats slash dates as day/month/year to avoid UAE/Philippines confusion.
  ...
  // Last fallback for ISO strings such as 2026-06-01T00:00:00.000Z.
...
function toISODate(date) {
  const parsed = parseAppDate(date) || new Date();
  const safe = isSafeDate(parsed) ? parsed : new Date();
...
function formatDate(dateString) {
  ...
  try {
    ...
    console.warn("Invalid date skipped:", dateString, error);
    return "No date";
  }
}
