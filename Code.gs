...
function saveAllData_(data) {
  ...
  if (transactions.length > 0) {
    transactionSheet.getRange(2, 1, transactions.length, 5).setValues(transactions.map(transaction => [
      transaction.id || '',
      normalizeDate_(transaction.date),
...
function makeDate_(year, month, day) {
  ...
  if (!isSafeDate_(parsed)) return null;
  if (parsed.getFullYear() !== year || parsed.getMonth() !== month - 1 || parsed.getDate() !== day) return null;
  return parsed;
}

function formatSafeDate_(date) {
...
function normalizeDate_(value) {
  ...
  try {
    if (isSafeDate_(value)) {
      return formatSafeDate_(value);
    }

    // Google Sheets serial number date. Example: 46174 = 2026-06-01.
    if (typeof value === 'number' && isFinite(value)) {
      if (value > 30000 && value < 80000) {
        const parsed = new Date(Math.round((value - 25569) * 86400 * 1000));
        return isSafeDate_(parsed) ? formatSafeDate_(parsed) : '';
      }
      return '';
    }
    ...
    if (match) {
      const parsed = makeDate_(Number(match[1]), Number(match[2]), Number(match[3]));
      return parsed ? formatSafeDate_(parsed) : '';
    }

    // Accept Sheet display dates like 01/06/2026 as day/month/year.
    match = text.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
    if (match) {
      const parsed = makeDate_(Number(match[3]), Number(match[2]), Number(match[1]));
      return parsed ? formatSafeDate_(parsed) : '';
    }
